---
title: "Tuya AR331-WZ control via MQTT"
description: "Integrate your Tuya AR331-WZ via Zigbee2MQTT with whatever smart home infrastructure you are using without the vendor's bridge or gateway."
...
| Picture | ![Tuya AR331-WZ](https://www.zigbee2mqtt.io/images/devices/AR331-WZ.png) |
...
- `preset`: Mode of this device (similar to system_mode). To control publish a message to topic `zigbee2mqtt/FRIENDLY_NAME/set` with payload `{"preset": VALUE}` where `VALUE` is one of: `auto`, `manual`, `leave`, `comfort`, `eco`, `off`. Reading (`/get`) this attribute is not possible.
